...
class TestArrowConfigs(unittest.TestCase):
    def test_splits(self):
        folder_path = join(folder(__file__), "docs", "source")
        labels = ["tinker", "tailor", "soldier", "spy"]  # some labels
        dataset = {l: [] for l in labels}
        for i, f in enumerate(get_files_in_folder(folder_path, [".rst"], sort=True)):
            dataset[labels[i % len(labels)]].append(f)

        # we can split consumer really easily
        out = ArrowConfig(
            class_to_id={l:i for i,l in enumerate(labels)},
            n_bytes=1
        )._train_(
            fps = {k:v[:-1] for k,v in dataset.items()},
        )._test_(
            fps = {k:v[-1:] for k,v in dataset.items()},
        ).split(ArrowConsumer)
        print(out)
...
class TestArrowDataset(unittest.TestCase):
    def test_dataset_full(self):
        ...
        # create the dataset
        data = ArrowConsumer(
            ArrowConfig(dataset, seqlen=128, class_to_id={"tinker": 0, "tailor": 1, "soldier": 2, "spy": 3})
        )
        print(data)
...
class TestTrainer(unittest.TestCase):
    def test_trainer(self):
        ...
        # create the dataset
        data = ArrowConsumer(
            ArrowConfig(dataset, seqlen = 128, n_bytes=1, class_to_id=class_to_id)
        )
        ...
        pbar = trange(2000)
        acc_over_time = []
        optim = torch.optim.Adam(trainer.model.parameters(), lr = 0.0001)
        for i in pbar:
            meta = trainer(
