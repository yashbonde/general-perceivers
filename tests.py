...
class TestArrowDataset(unittest.TestCase):
    def test_dataset_full(self):
        ...
        out = data[None]
        self.assertTrue(set(out.keys()) == {"input_array", "attention_mask", "class"})
        self.assertEqual(out["input_array"].shape, (1, 1976))  # I0

        out = data[random.choice(range(len(data)))]
        self.assertTrue(set(out.keys()) == {"input_array", "attention_mask", "class"})
        self.assertEqual(out["input_array"].shape, (1, 1976))  # I1

        out = data[4:5]
        self.assertTrue(set(out.keys()) == {"input_array", "attention_mask", "class"})
        self.assertEqual(out["input_array"].shape, (1, 1976))  # I2 - A

        out = data[0:5]
        self.assertTrue(set(out.keys()) == {"input_array", "attention_mask", "class"})
        self.assertEqual(out["input_array"].shape, (5, 1976))  # I2 - B

        out = data[[1, 2, 3, 4, 5]]
        self.assertTrue(set(out.keys()) == {"input_array", "attention_mask", "class"})
        self.assertEqual(out["input_array"].shape, (5, 1976))  # I3

        # out = data[{"tinker": 1, "tailor": 2, "soldier": 1, "spy": 1}]
        # self.assertTrue(set(out.keys()) == {"input_array", "attention_mask", "class"})
        # self.assertEqual(out["input_array"].shape, (5, 1976))  # I4

        # out = data[{"tinker": [1, 2], "tailor": [0, 1], "soldier": [0], "spy": [1]}]
        # self.assertTrue(set(out.keys()) == {"input_array", "attention_mask", "class"})
        # self.assertEqual(out["input_array"].shape, (6, 1976))  # I5
        ...
        # self.assertEqual(out["output_array"].shape, (6, 1976))  # I5
