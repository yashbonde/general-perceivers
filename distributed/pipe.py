...
config = TextConfig(
  ...
  latent_frac = 0.25, # 75% reduction per layer
...
def train():
    ...
    for batch, (inputs, target, attn_mask) in enumerate(train_zip):
        ...
        output = model(*model_input).local_value()
...
def evaluate(eval_model, data_source):
    ...
    with torch.no_grad():
        for (inputs, target, attn_mask) in test_zip:
            ...
            output = eval_model(*model_input).local_value()
            logits = output.contiguous().reshape(-1, config.vocab_size)
            target = target.contiguous().reshape(-1).cuda(1)
            loss = F.cross_entropy(logits, target)
            loss.backward()
            # Need to move targets to the device where the output of the
            # pipeline resides.
            total_loss += len(data) * criterion(output_flat, targets.reshape(-1).cuda(1)).item()
