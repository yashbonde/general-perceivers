...
def become_server(host: str = "0.0.0.0", port: int = 6969, reload: bool = False, log_level: str = "warning"):
    """
    ...
    ALL_ENDPOINTS.append(
        {"path": "/", "endpoint": __ping_method, "status_code": 200, "response_model": response_model, "name": None, "methods": ["GET"]}
    )
...
def status():
    ...
    for a in ALL_ENDPOINTS:
        all_methods.append(
            {"path": a["path"], "methods": a["methods"], "endpoint": a["endpoint"].__name__, "status_code": a["status_code"]}
        )
