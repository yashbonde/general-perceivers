...
class GrokkingTrainer():
  ...
  def train(self, verbose = True):
    print("> > starting process for model_id:", self.model_id)
    print("> > PID:", os.getpid())

    # f = open("./smaple.asdasdfasdfsdaf", "wb")
    ...
    print(f"> > [{self.model_id}] loss: {np.mean(all_loss[-50:]):.4f} | acc: {all_acc[-1]:.4f} | loss_test: {np.mean(all_loss_test):.4f} | acc_test: {np.mean(all_acc_test):.4f}")
